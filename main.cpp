    ...
    switch (code)
    {
                ...
                case SVC:
                    ...
                    op=GetIntOpForRR(c,regs);
                        ...
                        {
                            case 0: exit(0); break;
                            case 100: cin>>regs[reg1];  break;
                            case 101: cin>>d1.d; regs[reg1]=d1.i;  break;
                            case 102: cout<<regs[reg1]; break;
                            case 104: cin>>regs[reg1];  break;
                            case 103: d1.i = regs[reg1]; cout<<d1.d; break;
                            case 105: cout<<(char)regs[reg1];  break;
                        }
                    }
                    break;
                case ADD:
                ...
                case CMP:
                    regs[31]+=4; //> = 1  < = 0 = = 2
                        reg1 = (c>>21)&(0x1F);
                        reg2 = (c>>16)&(0x1F);
                        if(regs[reg1]>regs[reg2]+GetIntOpForRR(c,regs))
                        regs[32]=1;
                        if(regs[reg1]<regs[reg2]+GetIntOpForRR(c,regs))
                        regs[32]=0;
                        if(regs[reg1]==regs[reg2]+GetIntOpForRR(c,regs))
                ...
                case CMPD:
                    regs[31]+=4; //> = 1  < = 0 = = 2
                       ...
                       // cout<<"chisla "<<d1.d<<" "<<d2.d;
                        if(d1.d>d2.d+GetDoubleOpForRRD(c,regs))
                        regs[32]=1;
                        if(d1.d<d2.d+GetDoubleOpForRRD(c,regs))
                        regs[32]=0;
                        if(d1.d==d2.d+GetDoubleOpForRRD(c,regs))
...
vector<string> Split(string s, map<string, unsigned int> &marks, vector<string> &nonstopinst) //the original idea belongs to Babichev S. patent issued 04.31.2020 #24234312397296
{
    static int pos = 0;
    //cout<<s<<endl;
    string cur="";
...
int main()
{
    ...
    long long int registres[33]; for(int i = 0; i<33; i++) registres[i]=0;
  ...
  //  for(auto c:marks) cout<<c.first<<" "<<c.second<<endl;
   // for(auto c:nonstopinstr) cout<<c<<endl;
    for(int i=0;i<nonstopinstr.size();)
    {
        ...
        if(cur == "end") break;
        if(cur == "word") {i++; code = stoi(nonstopinstr[i]); Instruct_to_mem(code,memor); i++; continue;}
        if(cur == "bytes") {i++; i++; continue;}
